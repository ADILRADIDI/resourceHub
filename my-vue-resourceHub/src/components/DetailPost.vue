<template>
  <div class="post-detail-wrapper flex flex-col md:flex-row gap-6 w-[80%] justify-center mx-auto">
    ...
    <div class="post-and-comments flex-1 max-w-4xl">
      ...
      <div class="post-detail-container bg-white p-6 rounded-lg shadow-lg mb-6 mt-5">
        ...
        <div class="post-header flex items-center mb-4">
          <img
            :src="post.user?.profile_picture ? `http://localhost:8000/storage/uploads/profile_pictures/${post.user.profile_picture}` : Image_Unkown_user"
            :alt="`${post.user?.name || 'Unknown User'} profile picture`"
        ...
        <div class="post-content text-gray-700">
          ...
          <div v-if="post.image" class="post-image mb-4">
            <img :src="`http://localhost:8000/storage/${post.image}`" alt="Post Image" class="w-full rounded-lg shadow-md">
          </div>
          <div v-if="post.video" class="post-video mt-4 flex justify-center">
            <video controls :src="`${API_BASE_URL_WITHOUT}storage/${post.video}`" class="w-full lg:w-3/4 rounded-lg shadow-md"></video>
          </div>
          <div v-if="post.pdf" class="post-pdf mt-4">
            <!-- uploads/pdfs/5tMXfMmk3IXe0jVaKZJ8ZtO91KedZsi6nv4aJqfJ.pdf -->
            <!-- :src="`http://localhost:8000/storage/uploads/pdfs/5tMXfMmk3IXe0jVaKZJ8ZtO91KedZsi6nv4aJqfJ.pdf`"  -->
            <!-- :src="`http://localhost:8000/storage/uploads/pdfs/${post.document}#toolbar=0`"  -->
            <iframe 
              :src="`http://localhost:8000/storage/${post.pdf}#toolbar=0`"
              frameborder="0"
    ...
    <div v-for="comment in comments" :key="comment.id" class="comment mb-6 bg-white border border-gray-200 px-5 py-4 rounded-lg shadow-sm transition-transform transform hover:scale-105">
        <div class="flex items-center justify-between mb-2">
            <div class="flex items-center gap-4">
              <img
                  :src="comment.profile_image ? `http://localhost:8000/storage/uploads/profile_pictures/${comment.profile_image}` : Image_Unkown_user"
                  :alt="`profile picture`"
                  class="rounded-full mx-3 w-10 h-10 shadow-md"
              />
                <div>
                    <router-link :to="`/u/${comment.user_id}`" class="font-semibold text-blue-600 hover:text-blue-800">
...
const fetchComments = async (postId) => {
  try {
    ...
    console.log('image comments profile --> ',comments.profile_picture);
  } catch (error) {
