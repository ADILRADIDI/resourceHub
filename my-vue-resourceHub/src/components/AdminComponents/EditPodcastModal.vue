...
<template>
    <div v-if="isOpen" class="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div class="bg-white p-6 rounded-lg shadow-xl z-10 w-full max-w-lg">
        <h2 class="text-xl font-semibold mb-4">Edit Podcast</h2>
        <form @submit.prevent="updatePodcast">
          <!-- Podcast Title -->
          <div class="mb-4">
          ...
          <div class="flex justify-end">
            <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded-lg">Update Podcast</button>
          </div>
        </form>
      </div>
    </div>
    ...
    methods: {
      ...
      async updatePodcast() {
        ...
        try {
          ...
          await axios.put(`/api/podcasts/${this.podcast.id}`, formData, {
            headers: {
